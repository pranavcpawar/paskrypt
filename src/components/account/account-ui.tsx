...
export function AccountBalance({ address }: { address: PublicKey }) {
	...
	return (
		<div>
			<h1
				className="text-5xl font-bold font-unbounded cursor-pointer"
				onClick={() => query.refetch()}
			>
				{query.data ? <BalanceSol balance={query.data} /> : "NA"} SOL
			</h1>
		</div>
	);
}
		...
		<div>
			...
			<div className="space-x-2 flex items-center justify-center w-full">
				<button
					...
					className="btn bg-[#0A67C2] hover:bg-[#0A67C2]/60 text-[#dadada] btn-xs lg:btn-wide lg:h-8"
...
export function AccountTokens({ address }: { address: PublicKey }) {
	...
	return (
		<div className="space-y-2 bg-black p-2 rounded-md border-2 border-[#252525]">
				...
				<div className="flex justify-between">
					...
					<div className="space-x-2">
						{query.isLoading ? (
							...
							<button
								className="btn btn-xs btn-circle bg-[#0A67C2] hover:bg-[#0A67C2]/60 text-black"
								onClick={async () => {
									await query.refetch();
							...
							<tbody>
								...
								{(query.data?.length ?? 0) > 5 && (
									<tr>
										<td colSpan={4} className="text-center">
											<button
												className="btn bg-[#0A67C2] hover:bg-[#0A67C2]/60 text-black btn-xs"
												onClick={() => setShowAll(!showAll)}
											>
												{showAll ? "Show Less" : "Show All"}
											</button>
